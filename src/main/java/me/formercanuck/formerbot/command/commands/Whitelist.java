        ...
        if (Main.getInstance().getBot().getChannel().isMod(sender) || bot.getChannel().isWhiteListed(sender)) {
            if (args.size() > 0) {
                ...
                if (!bot.getChannel().isWhiteListed(user)) {
                    ...
                    Main.getInstance().getBot().getChannel().messageChannel(String.format("%s, successfully added %s to the whitelist", sender, args.get(0)));
                    return;
                }
                ...
                if (bot.getChannel().getWhitelisted().contains(user)) {
                    ...
                    config.set("whitelist", bot.getChannel().getWhitelisted());
                    Main.getInstance().getBot().getChannel().messageChannel(String.format("%s, successfully removed %s from the whitelist", sender, args.get(0)));
                }
            } else {
